...
$$
P(Y=y_i)=\sum P(g(X)=y_i)
$$

### 连续随机变量

如果 $X$ 的概率密度函数为 $f(x)$，随机变量 $Y=g(X)$，且 $g(x)$ 逐段单调。

所谓逐段单调，就是在定义域上能做出一个分划，使得 $g(x)$ 在每一个分划出的子区间上单调，记每段单调的函数为 $g_i(x)$，那么其存在反函数 $h_i(x)$。

那么 $Y$ 的概率密度函数为

$$
p_Y(y)=\sum_j f(h_j(x))|h'_j(x)|
$$

### 随机变量的线性变换

如果 $X$ 的概率密度函数为 $f(x)$，随机变量 $Y=aX+b$。

那么 $Y$ 的概率密度函数为

$$
p_Y(y)=\dfrac1af(\dfrac{y-b}{a})
$$
...
$$
P(X+Y+Z<t)=\int_{-\infty}^{+\infty} (f\circ g)'(s) h(t-s)\mathrm ds
$$

如果不满足相互独立，那么就要使用含参重积分

$$
P(X+Y+Z<t)=\iiint_{D(t)} f(x,y,z)\mathrm d\sigma
$$
...
如果独立的随机变量序列$\{X_n\}$满足[林德伯格条件](https://mathworld.wolfram.com/LindebergCondition.html)，那么
...
1. $B=\sqrt{\sum_{i=1}^n Var(X_i)}$
2. $\mu_i=EX_i$

由于林德伯格条件比较繁杂，而且在实际的应用上较难验证，在此不赘述，感兴趣的读者可以移步至[MathWorld](https://mathworld.wolfram.com/LindebergCondition.html)查看。
